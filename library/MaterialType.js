/*
  __  __       _            _       _ _______               
 |  \/  |     | |          (_)     | |__   __|              
 | \  / | __ _| |_ ___ _ __ _  __ _| |  | |_   _ _ __   ___ 
 | |\/| |/ _` | __/ _ \ '__| |/ _` | |  | | | | | '_ \ / _ \
 | |  | | (_| | ||  __/ |  | | (_| | |  | | |_| | |_) |  __/
 |_|  |_|\__,_|\__\___|_|  |_|\__,_|_|  |_|\__, | .__/ \___|
                                            __/ | |         
                                           |___/|_|         
...
let Material = function(type) {
	...
	if (type instanceof String) {
		...
		this.liquid = type.liquid;
	}
};
Material.prototype.getIds = function() {
	return this.ids;
};
Material.prototype.add = function(id) {
	if (!this.has(id)) {
		this.getIds().push(id);
		return true;
	}
...
Material.prototype.addAll = function(ids) {
	let result = false;
	if (ids instanceof Array) {
		for (let i = 0; i < ids.length; ids++) {
			if (result) this.add(ids[i]);
			else result = this.add(ids[i]);
		}
	}
	return result;
};
Material.prototype.has = function(id) {
	return this.getIds().indexOf(id) != -1;
};
Material.prototype.hasAll = function(ids) {
	if (ids instanceof Array) {
		for (let i = 0; i < ids.length; i++) {
			if (!this.has(ids[i])) return false;
		}
		return true;
	}
	return false;
};
Material.prototype.remove = function(id) {
	if (this.has(id)) {
...
Material.prototype.removeAll = function() {
	this.ids = new Array();
};
...
Material.prototype.isSolid = function() {
	return this.solid;
};
...
Material.createFromId = function(type, baseId) {
	if (MaterialType.hasMaterial(type)) {
		return MaterialType.getMaterial(type);
	}
	if (baseId instanceof String) {
		baseId = Block.getNumericId(baseId);
	}
	if (baseId == undefined || baseId == null) {
		return null;
	}
	if (LiquidRegistry.isExists(type)) {
		return Material.create(type, Block.isNativeTile(baseId), Block.isSolid(baseId), true);
	}
	return Material.create(type, Block.isNativeTile(baseId), Block.isSolid(baseId));
};
...
MaterialType.hasMaterial = function(typeOrMaterial) {
	...
	return this.getMaterials().hasOwnProperty(typeOrMaterial);
};
...
MaterialType.registerMaterial = function(type, baseId) {
	if (this.hasMaterial(type)) {
		return null;
	}
	if (baseId instanceof Number || baseId instanceof String) {
		this.addMaterial(Material.createFromId(type, baseId));
	} else {
